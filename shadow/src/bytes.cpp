...
bytes &bytes::cover(const shadow::bytes &data, std::size_t begin) {
...
bytes bytes::split(std::size_t begin, std::size_t end) {
    if (end == std::size_t(-1)||end>this->size()) {
        end = this->size();
    }
    if(end<begin){
        return bytes();
...
std::ostream &operator<<(std::ostream &out,const shadow::bytes &data) {
#ifdef HEX
    out << std::hex;
    for (const auto &i:data) {
        out << int(i);
